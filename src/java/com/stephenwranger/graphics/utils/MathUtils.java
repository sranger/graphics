...
public class MathUtils {
   ...
   public static double[] computeNormal(final double[][] vertices, final int[] indices) {
      final Vector3d e1 = new Vector3d(vertices[indices[1]][0] - vertices[indices[0]][0], vertices[indices[1]][1] - vertices[indices[0]][1], vertices[indices[1]][2] - vertices[indices[0]][2]);
      final Vector3d e2 = new Vector3d(vertices[indices[2]][0] - vertices[indices[0]][0], vertices[indices[2]][1] - vertices[indices[0]][1], vertices[indices[2]][2] - vertices[indices[0]][2]);
   ...
   public static BoundingBox getKeyframeBounds(final List<Keyframe> keyframes, final double oversize) {
      ...
      return new BoundingBox(minX - ((maxX - minX) * (oversize - 1.0)), minY - ((maxY - minY) * (oversize - 1.0)), minZ - ((maxZ - minZ) * (oversize - 1.0)), maxX + ((maxX - minX) * (oversize - 1.0)),
            maxY + ((maxY - minY) * (oversize - 1.0)), maxZ + ((maxZ - minZ) * (oversize - 1.0)));
   }
