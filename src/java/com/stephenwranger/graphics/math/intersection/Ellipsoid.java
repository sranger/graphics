...
public class Ellipsoid {
   ...
   public Ellipsoid(final Tuple3d center, final double semiMajorAxis, final double flattening, final double firstEccentricitySquared, final double secondEccentricitySquared) {
      ...
      this.xAxisScale = 1.0;
      this.yAxisScale = 1.0;
      this.zAxisScale = this.invFlattening;
      this.xImplicitCoeff = 1.0 / (this.xAxisScale * this.xAxisScale);
      this.yImplicitCoeff = 1.0 / (this.yAxisScale * this.yAxisScale);
      this.zImplicitCoeff = 1.0 / (this.zAxisScale * this.zAxisScale);
   }

   /**
    * Intersection test with this {@link Ellipsoid}. Returns zero, one, or two intersections; zero means no
    * intersection,
   ...
   public double[] getIntersection(final PickingRay ray) {
      ...
      final double diffX = origin.x - this.center.x;
      final double diffY = origin.y - this.center.y;
      final double diffZ = origin.z - this.center.z;

      final double a = (this.xImplicitCoeff * dir.x * dir.x) + (this.yImplicitCoeff * dir.y * dir.y) + (this.zImplicitCoeff * dir.z * dir.z);

      final double b = 2.0 * ((this.xImplicitCoeff * diffX * dir.x) + (this.yImplicitCoeff * diffY * dir.y) + (this.zImplicitCoeff * diffZ * dir.z));

      final double c = ((this.xImplicitCoeff * diffX * diffX) + (this.yImplicitCoeff * diffY * diffY) + (this.zImplicitCoeff * diffZ * diffZ)) - (this.semiMajorAxis * this.semiMajorAxis);
   ...
   public Tuple3d intersectionToLonLat(final PickingRay ray, final double distance) {
      final Tuple3d translatedPt = this.intersectionToXYZ(ray, distance);

      return this.toLonLatAlt(translatedPt);
   }
   ...
   public Tuple3d intersectionToXYZ(final PickingRay ray, final double distance) {
      ...
      translatedPt.subtract(intersectionPt, this.center);
   ...
   public Tuple3d toLonLatAlt(final Tuple3d cartesian) {
      ...
      if (cartesianOffset.x != 0) {
         ...
         if (cartesianOffset.y > 0) {
            ...
            if (cartesianOffset.z > 0) { /* north pole */
               lat = Ellipsoid.MAX_LAT;
            } else if (cartesianOffset.z < 0) { /* south pole */
               lat = Ellipsoid.MIN_LAT;
      ...
      final double t0 = cartesianOffset.z * Ellipsoid.AD_C; // initial estimate of vertical component
      ...
      if (cosP1 >= Ellipsoid.COS_67P5) {
         alt = (w / cosP1) - Rn;
      } else if (cosP1 <= -Ellipsoid.COS_67P5) {
         alt = (w / -cosP1) - Rn;
      } else {
   ...
   public Tuple3d toXYZ(final Tuple3d lonLatAltDeg) {
      ...
      final double Rn = this.semiMajorAxis / Math.sqrt(1 - (this.firstEccentricitySquared * sin2Lat));  // earth radius at location

      final Tuple3d result = new Tuple3d();

      result.x = (Rn + alt) * cosLat * Math.cos(lon);
      result.y = (Rn + alt) * cosLat * Math.sin(lon);
      result.z = ((Rn * (1 - this.firstEccentricitySquared)) + alt) * sinLat;
