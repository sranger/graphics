...
public class SphericalCoordinate {
   ...
   public void update(final double stepAzimuthRads, final double stepElevationRads, final double stepRange) {
      ...
      this.range = Math.max(0.1, this.range + stepRange);
   }
   ...
   public static Quat4d getOrientation(final double azimuthRads, final double elevationRads) {
      ...
      orientation.mult(new Quat4d(SphericalCoordinate.AZIMUTH_ROTATION_AXIS, Math.toDegrees(azimuthRads)));
      orientation.mult(new Quat4d(SphericalCoordinate.ELEVATION_ROTATION_AXIS, Math.toDegrees(elevationRads)));
